...
export function StatusDistributionPie({ jobs }: { jobs: JobItem[] }) {
  ...
  return (
    <div className="rounded-[20px] bg-white p-5 dark:bg-[#1f1f1f]">
      <h4 className="mb-1 text-sm font-semibold text-[#1a1a1a] dark:text-[#f5f5f5]">状态分布</h4>
      <p className="mb-3 text-xs text-[#666666] dark:text-[#b8b8b8]">按全量岗位状态实时统计</p>
      {data.length === 0 ? (
        <p className="text-xs text-[#888888] dark:text-[#9a9a9a]">暂无岗位数据。</p>
      ) : (
        <>
          ...
          <ul className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs">
            {data.map((item) => (
              <li key={item.name} className="flex items-center justify-between rounded-lg bg-[#fbfaf8] px-2.5 py-1.5 dark:bg-[#2a2a2a]">
                <span className="inline-flex items-center gap-1.5 text-[#666666] dark:text-[#b8b8b8]">
                  <span className="inline-block h-2.5 w-2.5 rounded-full" style={{ backgroundColor: item.color }} />
                  {item.name}
                </span>
                <span className="font-semibold text-[#1a1a1a] dark:text-[#f5f5f5]">{item.value}</span>
              </li>
            ))}
          </ul>
        </>

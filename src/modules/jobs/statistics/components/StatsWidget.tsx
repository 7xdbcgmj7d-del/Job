...
function StatItem({ label, value, color }: StatItemProps) {
  return (
    <div className="flex flex-col items-center gap-1">
      <div
        className="text-2xl font-extrabold text-[#1a1a1a] dark:text-[#f5f5f5]"
        style={color ? { color } : undefined}
      >
        {value}
      </div>
      <div className="text-xs text-[#666666] dark:text-[#b8b8b8]">{label}</div>
    </div>
  )
}
...
export function StatsWidget({ jobs }: { jobs: JobItem[] }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-4 rounded-[20px] bg-white p-5 dark:bg-[#1f1f1f] sm:grid-cols-4">

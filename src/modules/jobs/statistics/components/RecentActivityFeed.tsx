...
export function RecentActivityFeed({
 ...
 <h4 className="mb-3 text-sm font-semibold text-[#1a1a1a]">最近动态</h4>
 <ul className="space-y-2.5">
 {rows.length === 0 ? (
 <li className="text-xs text-[#888888]">暂无投递或面试事件。</li>
 ) : (
 rows.map((row) => (
 <li key={row.id} className="rounded-xl bg-[#fbfaf8] px-3 py-2 text-xs text-[#1a1a1a]">
 ...
 <span className="text-[#666666]"> — {row.subtitle}</span>
 </li>
 ))
 )}
 </ul>

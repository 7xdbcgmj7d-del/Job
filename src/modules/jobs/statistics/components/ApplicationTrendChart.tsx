...
export function ApplicationTrendChart({ jobs }: { jobs: JobItem[] }) {
  ...
  return (
    <div className="rounded-[20px] bg-white p-5 dark:bg-[#1f1f1f]">
      <h4 className="mb-1 text-sm font-semibold text-[#1a1a1a] dark:text-[#f5f5f5]">近 30 日投递趋势</h4>
      <p className="mb-3 text-xs text-[#666666] dark:text-[#b8b8b8]">按岗位「投递日」汇总（非待投递且有投递日）</p>
              ...
              content={({ active, payload }) => {
                ...
                return (
                  <div className="text-xs">
                    <div className="font-medium text-[#1a1a1a] dark:text-[#f5f5f5]">{row.date}</div>
                    <div className="text-[#666666] dark:text-[#b8b8b8]">投递 {row.count} 次</div>
                  </div>
                )
              }}
            />

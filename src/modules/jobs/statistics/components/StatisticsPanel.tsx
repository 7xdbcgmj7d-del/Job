...
function DecisionAlerts({ jobs }: { jobs: JobItem[] }) {
  ...
  return (
    <div className="rounded-[20px] bg-white p-5 dark:bg-[#1f1f1f]">
      <h4 className="mb-2 text-sm font-semibold text-[#1a1a1a] dark:text-[#f5f5f5]">决策提醒</h4>
      {staleApplied.length > 0 ? (
        <p className="text-xs text-[#5b4b00] dark:text-[#f0cf6f]">
          已投递超过 7 天未跟进岗位 {staleApplied.length} 个，建议今天优先跟进。
        </p>
      ) : (
        <p className="text-xs text-[#666666] dark:text-[#b8b8b8]">暂无超期未跟进岗位，节奏健康。</p>
      )}
    </div>
  )
}
...
export function StatisticsPanel({
  ...
  return (
    <Tabs defaultValue="distribution" className="w-full gap-3">
      <TabsList className="h-auto min-h-9 w-full flex-wrap bg-white/90 p-1 dark:bg-[#2a2a2a]">
        <TabsTrigger
          value="distribution"

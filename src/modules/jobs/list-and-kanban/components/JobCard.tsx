  ...
  return (
    ...
    >
      <div className="mb-3 flex items-start justify-between gap-2">
        <div className="rounded-full bg-white/60 px-3 py-1 backdrop-blur-sm dark:bg-black/20">
        ...
        {hasResume || hasValidSourceUrl ? (
          hasValidSourceUrl ? (
            <button
              ...
              aria-label="打开岗位来源链接"
              title="打开岗位来源链接"
              className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-full bg-white/60 transition hover:bg-white/85 dark:bg-black/20 dark:hover:bg-black/35"
            >
      ...
      {recommendationLabel && onRecommendedAction && id !== undefined ? (
        <button
          type="button"
          className="mb-3 w-full rounded-full border border-[#1a1a1a]/20 bg-white/65 py-1.5 text-xs font-semibold text-[#1a1a1a] transition hover:bg-white dark:bg-black/15"
          onClick={(e) => {
            e.stopPropagation()
        ...
        <div className="flex gap-2 border-t border-black/5 pt-3">
          {onEdit ? (
            <button
              ...
              className="flex flex-1 items-center justify-center gap-1 rounded-full bg-white/70 py-2 text-xs font-semibold text-[#1a1a1a] transition-colors hover:bg-white dark:bg-black/20 dark:hover:bg-black/30"
            >
              <Pencil size={14} />
              编辑
            </button>
          ...
          {onDelete ? (
            <button
              ...
              className="flex flex-1 items-center justify-center gap-1 rounded-full bg-white/70 py-2 text-xs font-semibold text-red-700 transition-colors hover:bg-white dark:bg-black/20 dark:text-red-400 dark:hover:bg-black/30"
            >
              <Trash2 size={14} />
              删除
            </button>
